...
**Weekly start limit is league-level metadata.** `LeagueSettings.weeklyStartLimit` is optional and independent from roster slots. It does not change raw point scoring directly; PAR uses it to apply a start-scarcity cost to pitchers with projected starts and to create relief-side replacement demand from flexible over-cap starter capacity.
