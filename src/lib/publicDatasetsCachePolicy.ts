...
//
// Ground truth (observed on the production Next server, `next start`):
...
//
// On the TanStack Start side these headers are applied at the SERVING layer
// (vite dev/preview middleware today; the phase-5 production host — Nitro
// routeRules, CDN rules, or equivalent — must apply the same values). They
// are deliberately NOT set inside the API handlers: the contract fixtures in
// src/test/contracts/publicDatasets/ lock handler headers to content-type
// only, keeping handler responses byte-identical across frameworks.
//
