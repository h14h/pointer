...
describe("settings sections", () => {
  ...
  it("keeps open-panel keeper row order frozen when a round edit would re-sort by round", async () => {
    const user = userEvent.setup();
    useStoreMock.mockReturnValue({
      ...createStoreState(),
      projectionGroups: [
    ...
    await user.click(screen.getByRole("button", { name: "Show keepers for Team 1" }));
    const rowsBefore = Array.from(document.querySelectorAll("[data-keeper-row]"));
    expect(rowsBefore.map((node) => node.getAttribute("data-keeper-row"))).toEqual([
      "keeper-1",
      "keeper-2",
    ]);
    ...
    const rows = Array.from(document.querySelectorAll("[data-keeper-row]"));
    expect(rows.map((node) => node.getAttribute("data-keeper-row"))).toEqual([
      "keeper-1",
      "keeper-2",
    ]);
    expect(screen.getByLabelText("Keeper round for Mike Trout")).toHaveValue("4");
    expect(screen.getByText("Mookie Betts").closest("[data-keeper-row]")).not.toHaveClass(
      "keeper-row-fade-in"
    );
  });
