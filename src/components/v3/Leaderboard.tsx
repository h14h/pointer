...
export function Leaderboard() {
	...
	return (
		<div className="flex flex-col">
			{/* Filters */}
			<div className="mb-4 rounded-2xl border-2 border-purple-200 dark:border-purple-800/60 bg-white dark:bg-[#1a1030] p-4 shadow-lg shadow-purple-500/5 dark:shadow-purple-500/10">
				<div className="flex flex-wrap items-center gap-3">
					<input
						...
						className="w-full min-w-[220px] flex-1 rounded-lg border-2 border-purple-200 dark:border-purple-700 bg-white dark:bg-[#0f0a1a] px-3 py-1.5 text-sm font-medium text-[#1e1e2e] dark:text-[#f0f0f0] placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-purple-500 dark:focus:border-purple-400 focus:ring-2 focus:ring-purple-200 dark:focus:ring-purple-800"
					/>
					...
					<select
						...
						className="rounded-lg border-2 border-purple-200 dark:border-purple-700 bg-white dark:bg-[#0f0a1a] px-3 py-1.5 text-sm font-semibold text-[#1e1e2e] dark:text-[#f0f0f0] focus:border-purple-500 dark:focus:border-purple-400 focus:ring-2 focus:ring-purple-200 dark:focus:ring-purple-800"
					>
					...
					{projectionGroups.length > 1 && (
						<div className="flex items-center gap-2">
							...
							{isSwitchingGroups && (
								<span
									className="h-4 w-4 animate-spin rounded-full border-2 border-purple-200 dark:border-purple-700 border-t-purple-600"
					...
					<button
						...
						className="rounded-lg border-2 border-purple-300 dark:border-purple-700 bg-white dark:bg-[#1a1030] px-3 py-1.5 text-sm font-bold uppercase tracking-wider text-purple-700 dark:text-purple-300 hover:bg-purple-50 dark:hover:bg-purple-950/50 transition-colors"
			...
			{isStatsOpen && (
				<div
					...
					className="rounded-2xl border-2 border-purple-200 dark:border-purple-800/60 bg-white dark:bg-[#1a1030] p-4 shadow-lg shadow-purple-500/5 dark:shadow-purple-500/10"
				>
					<div className="flex flex-wrap items-center justify-between gap-3">
						...
						<button
							...
							className="rounded-lg border-2 border-purple-300 dark:border-purple-700 px-3 py-1 text-xs font-bold uppercase tracking-wider text-purple-700 dark:text-purple-300 hover:bg-purple-50 dark:hover:bg-purple-950/50 transition-colors"
						>
							Close
						</button>
					</div>
					<div className="mt-4 grid gap-4 lg:grid-cols-2">
						<div className="rounded-xl bg-[#f0f4ff] dark:bg-[#0f0a1a] p-3 border border-blue-200 dark:border-blue-900/50">
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center gap-2">
									<button
										...
										className="text-xs font-bold uppercase tracking-wider text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300"
									>
										All
									</button>
									<button
										onClick={() => clearAllStats("batting")}
										className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
									>
										None
									</button>
								</div>
							...
							<div className="mt-3 flex flex-wrap gap-2">
								{BATTING_STAT_OPTIONS.map((stat) => (
									<label
										key={stat.id}
										className="flex items-center gap-2 rounded-lg border-2 border-purple-200 dark:border-purple-800/60 bg-white dark:bg-[#1a1030] px-2.5 py-1.5 text-xs font-bold text-[#1e1e2e] dark:text-[#f0f0f0] shadow-sm"
									>
										<input
											type="checkbox"
											...
											className="h-3.5 w-3.5 rounded border-purple-300 dark:border-purple-700 text-purple-600 focus:ring-purple-300 dark:focus:ring-purple-800"
										/>
						...
						<div className="rounded-xl bg-[#f0f4ff] dark:bg-[#0f0a1a] p-3 border border-purple-200 dark:border-purple-900/50">
							<div className="flex items-center justify-between">
			...
			<div className="relative">
				{isSwitchingGroups && (
					<div className="pointer-events-none absolute inset-0 rounded-2xl bg-purple-900/10 dark:bg-purple-900/20" />
				)}
				<LeaderboardTable
...
const LeaderboardTable = memo(function LeaderboardTable({
	...
	const columns = useMemo<ColumnDef<RankedPlayer>[]>(() => {
		...
		const baseColumns: ColumnDef<RankedPlayer>[] = [
			...
			},
			{
					...
					<div className="flex items-center gap-2 min-w-0">
						{isDraftMode && (
							<input
								...
								className="h-4 w-4 rounded border-purple-300 dark:border-purple-700 text-purple-600 focus:ring-purple-300 dark:focus:ring-purple-800"
						...
						<span
							className={
								row.original.isDrafted
									? "text-slate-400 dark:text-slate-500 line-through truncate"
									: row.original.isKeeper
							? "font-black text-amber-600 dark:text-amber-300 truncate"
				...
				>
					{row.original.player.Name}
						...
						{row.original.isDrafted && (
							<span className="rounded-full bg-blue-100 dark:bg-blue-950/50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
								{resolveTeamLabel(row.original.draftedTeamIndex)}
							</span>
						)}
						{row.original.isKeeper && (
							<span className="rounded-full bg-amber-100 dark:bg-amber-950/50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-amber-700 dark:text-amber-300 border border-amber-300 dark:border-amber-700">
						...
						className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
							row.original.player._type === "batter"
								...
								: row.original.player._type === "pitcher"
									...
									: "bg-gradient-to-r from-purple-100 to-blue-100 text-purple-700 dark:from-purple-950/50 dark:to-blue-950/50 dark:text-purple-300 border border-purple-200 dark:border-purple-800"
						}`}
					>
						{row.original.player._type === "batter"
							? "BAT"
			...
			},
			{
				...
				cell: ({ getValue }) => (
					<span className="text-xs font-medium text-[#1e1e2e] dark:text-[#f0f0f0]">
						{getValue() as string}
					</span>
				),
			},
			{
				...
				size: 95,
				cell: ({ row }) => (
					<span className="font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-blue-500 dark:from-purple-400 dark:to-blue-400">
		...
		const withLeadingSeparator = (
			...
			const mergedClass = [existingClass, "border-l-2 border-purple-200 dark:border-purple-800"]
				.filter(Boolean)
				.join(" ");
	...
	if (
		...
		return (
			<div className="flex h-96 flex-col items-center justify-center rounded-2xl border-2 border-dashed border-purple-300 dark:border-purple-800">
				<p className="mb-2 text-lg font-black uppercase tracking-wider text-purple-400 dark:text-purple-500">No players loaded</p>
				<p className="text-sm font-medium text-slate-500 dark:text-slate-400">Upload a CSV file to get started</p>
			</div>
		);
	}
	...
	return (
		<div className="space-y-3">
			<div className="overflow-x-auto rounded-2xl border-2 border-purple-200 dark:border-purple-800/60 bg-white dark:bg-[#1a1030] shadow-lg shadow-purple-500/5 dark:shadow-purple-500/10">
				<table className="w-full text-sm text-[#1e1e2e] dark:text-[#f0f0f0]">
					<thead>
						<tr className="border-t-4 border-t-transparent" style={{ borderImage: "linear-gradient(to right, #7c3aed, #3b82f6) 1" }}>
							<td colSpan={999} className="h-0 p-0" />
						</tr>
								{table.getHeaderGroups().map((headerGroup) => (
									<tr key={headerGroup.id} className="bg-purple-600 dark:bg-purple-900">
										{headerGroup.headers.map((header) => (
										<th
											key={header.id}
											style={{ width: header.getSize() }}
											className={`px-3 py-2.5 text-left text-[10px] font-black uppercase tracking-wider text-white dark:text-purple-100 whitespace-nowrap ${
												header.column.getCanSort()
													? "cursor-pointer select-none hover:text-purple-200"
													: ""
											} ${
													(
					...
					<tbody>
						{table.getRowModel().rows.map((row, index) => (
							<tr
								...
								className={`border-t border-purple-100 dark:border-purple-900/50 transition-colors ${
									...
									row.original.isDrafted
										...
										: row.original.isKeeper
											...
											: index % 2 === 0
												? "bg-white dark:bg-[#1a1030] hover:bg-purple-50 dark:hover:bg-purple-950/30"
												: "bg-[#f0f4ff] dark:bg-[#0f0a1a] hover:bg-purple-50 dark:hover:bg-purple-950/30"
								}`}
							>
			...
			<div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border-2 border-purple-200 dark:border-purple-800/60 bg-white dark:bg-[#1a1030] px-4 py-2.5 text-xs font-bold shadow-lg shadow-purple-500/5 dark:shadow-purple-500/10">
				<div className="flex items-center gap-2">
					<button
						onClick={() => table.previousPage()}
						disabled={!table.getCanPreviousPage()}
						className="rounded-lg border-2 border-purple-300 dark:border-purple-700 bg-white dark:bg-[#0f0a1a] px-3 py-1 text-xs font-bold uppercase tracking-wider text-purple-700 dark:text-purple-300 hover:bg-purple-50 dark:hover:bg-purple-950/50 disabled:cursor-not-allowed disabled:opacity-40 transition-colors"
					>
						Prev
					</button>
					<button
					...
					<span className="text-[#1e1e2e] dark:text-[#f0f0f0] uppercase tracking-wider">
				...
				<div className="flex items-center gap-3">
					...
					<label className="flex items-center gap-2">
						<span className="text-slate-500 dark:text-slate-400 uppercase tracking-wider">Rows</span>
						<select
							...
							className="rounded-lg border-2 border-purple-200 dark:border-purple-700 bg-white dark:bg-[#0f0a1a] px-2 py-1 text-xs font-bold text-purple-700 dark:text-purple-300 focus:border-purple-500 dark:focus:border-purple-400 focus:ring-2 focus:ring-purple-200 dark:focus:ring-purple-800"
						>
