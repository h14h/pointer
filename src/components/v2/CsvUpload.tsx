...
export function CsvUpload({ isOpen, onClose }: CsvUploadProps) {
  ...
  const renderPreview = (fileState: UploadFileState) => {
    ...
    return (
      <div className="mb-5 rounded-2xl border border-[#d4c4b0] dark:border-[#3d2b1f] bg-[#faf6f1] dark:bg-[#1a1410] p-5 shadow-lg shadow-amber-900/5">
        <p className="text-sm text-[#3d2b1f] dark:text-[#e8ddd0]" style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>
          Detected: <span className="font-semibold">{parseResult.rowCount} {parseResult.type}s</span>
        </p>

        {parseResult.errors.length > 0 && (
          <div className="mt-3">
            <p className="text-sm font-medium text-[#b87333] dark:text-[#d4a574]">
              {parseResult.errors.length} warning(s):
            </p>
            <ul className="mt-1 max-h-20 overflow-y-auto text-xs text-[#b87333] dark:text-[#d4a574]">
        ...
        <div className="mt-4">
          <p className="mb-2 text-sm font-semibold text-[#3d2b1f] dark:text-[#e8ddd0]" style={{ fontFamily: "Georgia, 'Palatino Linotype', Palatino, serif" }}>
            Preview (first 5):
          </p>
          <div className="max-h-40 overflow-y-auto rounded-xl border border-[#d4c4b0] dark:border-[#3d2b1f] bg-[#faf6f1] dark:bg-[#1a1410]">
            <table className="w-full text-xs text-[#3d2b1f] dark:text-[#e8ddd0]" style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>
              <thead className="bg-[#f0e6d8] dark:bg-[#2a1f18] text-[#6b5744] dark:text-[#a08b70]">
                <tr>
                  <th className="px-3 py-2 text-left font-semibold tracking-wide">Name</th>
                  <th className="px-3 py-2 text-left font-semibold tracking-wide">Team</th>
                  {parseResult.type === "batter" ? (
                    <>
                      <th className="px-3 py-2 text-right font-semibold tracking-wide">HR</th>
                      <th className="px-3 py-2 text-right font-semibold tracking-wide">R</th>
                      <th className="px-3 py-2 text-right font-semibold tracking-wide">RBI</th>
                    </>
                  ) : (
                    <>
                      <th className="px-3 py-2 text-right font-semibold tracking-wide">W</th>
                      <th className="px-3 py-2 text-right font-semibold tracking-wide">SO</th>
                      <th className="px-3 py-2 text-right font-semibold tracking-wide">ERA</th>
                    </>
                  )}
                </tr>
              </thead>
              <tbody className="text-[#3d2b1f] dark:text-[#e8ddd0]">
                {parseResult.players.slice(0, 5).map((p) => (
                  <tr
                    key={p._id}
                    className="border-t border-[#d4c4b0] dark:border-[#3d2b1f] odd:bg-[#faf6f1] dark:odd:bg-[#1a1410] even:bg-[#f0e6d8]/50 dark:even:bg-[#2a1f18]/50"
                  >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#3d2b1f]/20 dark:bg-[#0a0806]/60 backdrop-blur-sm">
      <div className="relative mx-0 h-full w-full max-w-none rounded-none border border-[#d4c4b0] dark:border-[#3d2b1f] bg-[#faf6f1] dark:bg-[#2a1f18] p-5 shadow-2xl overflow-y-auto sm:mx-4 sm:h-auto sm:max-h-[85vh] sm:max-w-lg sm:rounded-2xl sm:p-8">
        <button
          type="button"
          onClick={handleCancel}
          disabled={isImportingEligibility}
          aria-label="Close upload modal"
          className="absolute right-4 top-4 inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#d4c4b0] dark:border-[#3d2b1f] bg-[#faf6f1]/80 dark:bg-[#2a1f18]/80 text-[#8b7355] dark:text-[#a08b70] shadow-sm backdrop-blur transition hover:bg-[#f0e6d8] dark:hover:bg-[#3d2b1f] hover:text-[#3d2b1f] dark:hover:text-[#e8ddd0] disabled:cursor-not-allowed disabled:opacity-60 sm:right-8 sm:top-8"
        >
          <span className="text-lg leading-none">×</span>
        </button>
        <h2 className="mb-5 pr-10 text-lg font-bold text-[#3d2b1f] dark:text-[#e8ddd0]" style={{ fontFamily: "Georgia, 'Palatino Linotype', Palatino, serif" }}>
          Upload Player Projections
        </h2>

        {error && (
          <div className="mb-5 rounded-xl bg-[#fdf0ed] dark:bg-[#2a1510] border border-[#e0a89a] dark:border-[#5a2a1a] p-4 text-sm text-[#a0392a] dark:text-[#e8a090]" style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>
            {error}
          </div>
        )}
        ...
        {!batterFile && !pitcherFile ? (
          <div style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>
            <div className="mb-5">
              <label className="mb-2 block text-sm font-medium tracking-wide text-[#6b5744] dark:text-[#a08b70]">
                Player Type
              </label>
              <select
                value={uploadType}
                onChange={(e) => setUploadType(e.target.value as UploadType)}
                className="w-full rounded-xl border border-[#d4c4b0] dark:border-[#3d2b1f] bg-[#faf6f1] dark:bg-[#1a1410] px-4 py-2.5 text-sm text-[#3d2b1f] dark:text-[#e8ddd0] placeholder:text-[#b0a090] focus:border-[#c45d3e] dark:focus:border-[#d4704a] focus:ring-2 focus:ring-[#c45d3e]/20 dark:focus:ring-[#d4704a]/20"
              >
            ...
            <div
              ...
              onDrop={handleDrop}
              className={`mb-5 flex h-44 flex-col items-center justify-center rounded-2xl border-2 border-dashed transition-colors ${
                dragActive
                  ? "border-[#c45d3e] bg-[#c45d3e]/5 dark:bg-[#d4704a]/5"
                  : "border-[#d4c4b0] dark:border-[#3d2b1f] bg-[#f0e6d8]/30 dark:bg-[#2a1f18]/30"
              }`}
            >
              <p className="mb-2 text-sm text-[#6b5744] dark:text-[#a08b70]">
                Drag and drop CSV/TSV files here
              </p>
              <p className="mb-4 text-xs text-[#8b7355] dark:text-[#8b7355]">or</p>
              <label className="cursor-pointer rounded-full bg-[#c45d3e] px-6 py-2.5 text-sm font-semibold text-white shadow-md shadow-amber-900/10 hover:bg-[#b04e32] transition-colors">
                Browse Files
                <input
                  type="file"
                  accept=".csv,.tsv,.txt"
            ...
            <div className="flex justify-end">
              <button
                onClick={handleCancel}
                className="rounded-full px-5 py-2.5 text-sm text-[#6b5744] dark:text-[#a08b70] hover:bg-[#f0e6d8] dark:hover:bg-[#3d2b1f] transition-colors"
              >
                Cancel
              </button>
            </div>
          ...
          <div style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>
            <div className="mb-5 rounded-xl bg-[#f5e6c8] dark:bg-[#2a2010] border border-[#d4b88c] dark:border-[#4a3520] p-5">
              <p className="text-sm font-semibold text-[#8b6914] dark:text-[#d4a550]">
                No MLBAMID or PlayerId column found
              </p>
              <p className="mt-1 text-sm text-[#8b6914] dark:text-[#c4a060]">
            ...
            {batterFile?.parseResult.needsIdSelection && (
              <div className="mb-5">
                <p className="mb-2 text-sm font-semibold text-[#3d2b1f] dark:text-[#e8ddd0]">
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-[#d4c4b0] dark:border-[#3d2b1f] bg-[#faf6f1] dark:bg-[#1a1410] px-4 py-2.5 text-sm text-[#3d2b1f] dark:text-[#e8ddd0] focus:border-[#c45d3e] dark:focus:border-[#d4704a] focus:ring-2 focus:ring-[#c45d3e]/20 dark:focus:ring-[#d4704a]/20"
                >
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="rounded-full bg-[#c45d3e] px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-amber-900/10 hover:bg-[#b04e32] transition-colors"
              >
                Continue
              </button>
            </div>
          ...
          <div style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>
            <div className="mb-5">
              ...
              <input
                ...
                className="w-full rounded-xl border border-[#d4c4b0] dark:border-[#3d2b1f] bg-[#faf6f1] dark:bg-[#1a1410] px-4 py-2.5 text-sm text-[#3d2b1f] dark:text-[#e8ddd0] placeholder:text-[#b0a090] dark:placeholder:text-[#6b5744] focus:border-[#c45d3e] dark:focus:border-[#d4704a] focus:ring-2 focus:ring-[#c45d3e]/20 dark:focus:ring-[#d4704a]/20"
                placeholder="e.g. Steamer 2025"
              />
            </div>

            <div className="mb-5 rounded-2xl border border-[#d4c4b0] dark:border-[#3d2b1f] bg-[#f0e6d8]/50 dark:bg-[#2a1f18]/50 p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-[#3d2b1f] dark:text-[#e8ddd0]">
                    Import Position Eligibility
                  </p>
                  <p className="mt-1 text-xs text-[#6b5744] dark:text-[#a08b70]">
                ...
                <label className="relative inline-flex h-7 w-12 shrink-0 items-center">
                  ...
                  />
                  <span className="h-7 w-12 rounded-full bg-[#d4c4b0] dark:bg-[#3d2b1f] transition peer-checked:bg-[#c45d3e] dark:peer-checked:bg-[#d4704a] peer-disabled:opacity-60" />
              ...
              <p className="mt-3 text-xs text-[#8b7355] dark:text-[#8b7355]">
                May take a minute for larger files.
              </p>
            </div>

            {missingTypeWarning && (
              <div className="mb-5 rounded-xl bg-[#f5e6c8] dark:bg-[#2a2010] border border-[#d4b88c] dark:border-[#4a3520] p-4 text-sm text-[#8b6914] dark:text-[#d4a550]">
                {missingTypeWarning}
              </div>
            )}
            ...
            {(isImportingEligibility || importError) && (
              <div className="mb-5 rounded-2xl border border-[#d4c4b0] dark:border-[#3d2b1f] bg-[#faf6f1] dark:bg-[#1a1410] p-4 text-sm shadow-lg shadow-amber-900/5">
                {isImportingEligibility && (
                  <>
                      ...
                      return (
                        <>
                          <div className="mb-3 flex items-center justify-between">
                            <span className="font-medium text-[#3d2b1f] dark:text-[#e8ddd0]">
                          ...
                          <div
                            className="h-2.5 w-full overflow-hidden rounded-full bg-[#f0e6d8] dark:bg-[#3d2b1f]"
                            style={{
                              backgroundImage:
                                "linear-gradient(to right, #c45d3e, #c45d3e)",
                    ...
                    {retryStatus && (
                      <p className="mt-3 text-xs text-[#b87333] dark:text-[#d4a574]">
                        {retryStatus}
                      </p>
                    )}
                  </>
                ...
                {importError && !isImportingEligibility && (
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-[#a0392a] dark:text-[#e8a090]">
                      {importError}
                    </span>
                    <button
                      onClick={() => void handleRetryImport()}
                      className="rounded-full bg-[#a0392a]/10 dark:bg-[#a0392a]/20 px-4 py-1.5 text-xs font-semibold text-[#a0392a] dark:text-[#e8a090] hover:bg-[#a0392a]/20 dark:hover:bg-[#a0392a]/30 transition-colors"
                    >
                      Retry Import
                    </button>
                  </div>
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                disabled={isImportingEligibility}
                className="rounded-full px-5 py-2.5 text-sm text-[#6b5744] dark:text-[#a08b70] hover:bg-[#f0e6d8] dark:hover:bg-[#3d2b1f] disabled:cursor-not-allowed disabled:opacity-60 transition-colors"
              >
                Back
              </button>
              <button
                ...
                disabled={isImportingEligibility}
                className="rounded-full bg-[#c45d3e] px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-amber-900/10 hover:bg-[#b04e32] disabled:cursor-not-allowed disabled:opacity-60 transition-colors"
              >
                {isImportingEligibility
                  ? "Importing Group..."
