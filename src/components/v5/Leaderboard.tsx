...
export function Leaderboard() {
	...
	return (
		<div className="flex flex-col">
			{/* Filters */}
			<div className="mb-4 rounded-2xl border border-slate-200/50 dark:border-slate-600/30 backdrop-blur-xl bg-white/70 dark:bg-[#0f1729]/80 p-3 shadow-xl shadow-teal-500/5">
				<div className="flex flex-wrap items-center gap-3">
					<input
						...
						className="w-full min-w-[220px] flex-1 rounded-xl border border-slate-200/60 dark:border-slate-600/40 backdrop-blur-md bg-white/50 dark:bg-slate-800/50 px-3 py-1.5 text-sm text-[#1e293b] dark:text-[#cbd5e1] placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-teal-400 dark:focus:border-teal-400 focus:ring-2 focus:ring-teal-400/20 dark:focus:ring-teal-400/10 transition-all"
					/>

					<select
						value={playerView}
						onChange={(e) => setPlayerView(e.target.value as PlayerView)}
						className="rounded-xl border border-slate-200/60 dark:border-slate-600/40 backdrop-blur-md bg-white/50 dark:bg-slate-800/50 px-3 py-1.5 text-sm text-[#1e293b] dark:text-[#cbd5e1] focus:border-teal-400 dark:focus:border-teal-400 focus:ring-2 focus:ring-teal-400/20 dark:focus:ring-teal-400/10"
					>
					...
					{projectionGroups.length > 1 && (
						<div className="flex items-center gap-2">
							...
							{isSwitchingGroups && (
								<span
									className="h-4 w-4 animate-spin rounded-full border-2 border-slate-300/50 dark:border-slate-600/50 border-t-teal-500"
					...
					{isDraftMode && (
						<span className="text-xs font-light text-slate-400 dark:text-slate-500 tracking-wide">
					...
					<button
						...
						className="rounded-xl border border-slate-200/60 dark:border-slate-600/40 backdrop-blur-md bg-white/50 dark:bg-slate-800/50 px-3 py-1.5 text-sm font-medium text-[#1e293b] dark:text-[#cbd5e1] hover:bg-white/80 dark:hover:bg-slate-700/50 transition-all"
			...
			{isStatsOpen && (
				<div
					...
					className="mb-4 rounded-2xl border border-slate-200/50 dark:border-slate-600/30 backdrop-blur-xl bg-white/70 dark:bg-[#0f1729]/80 p-4 shadow-xl shadow-teal-500/5"
				>
					<div className="flex flex-wrap items-center justify-between gap-3">
						...
						<button
							...
							className="rounded-xl border border-slate-200/60 dark:border-slate-600/40 px-2.5 py-1 text-xs font-medium text-slate-500 dark:text-slate-400 hover:bg-white/60 dark:hover:bg-slate-800/50 transition-all"
						>
							Close
						</button>
					</div>
					<div className="mt-4 grid gap-4 lg:grid-cols-2">
						<div className="rounded-xl backdrop-blur-md bg-white/40 dark:bg-slate-800/40 border border-slate-200/40 dark:border-slate-600/20 p-3">
							...
							<div className="mt-3 flex flex-wrap gap-2">
								{BATTING_STAT_OPTIONS.map((stat) => (
									<label
										key={stat.id}
										className="flex items-center gap-2 rounded-xl border border-slate-200/50 dark:border-slate-600/30 backdrop-blur-md bg-white/50 dark:bg-slate-800/40 px-2.5 py-1.5 text-xs font-medium text-[#1e293b] dark:text-[#cbd5e1] shadow-sm"
									>
										<input
											type="checkbox"
											...
											className="h-3.5 w-3.5 rounded border-slate-300 dark:border-slate-600 text-teal-500 focus:ring-teal-400/30"
										/>
...
const LeaderboardTable = memo(function LeaderboardTable({
	...
	const columns = useMemo<ColumnDef<RankedPlayer>[]>(() => {
			...
			},
			{
					...
					<div className="flex items-center gap-2 min-w-0">
						{isDraftMode && (
							<input
								...
								className="h-4 w-4 rounded border-slate-300/60 dark:border-slate-600/40 text-teal-500 focus:ring-teal-400/30"
						...
						<span
							...
							? "font-medium text-amber-700 dark:text-amber-300 truncate"
							: "truncate"
					}
				...
				>
					{row.original.player.Name}
						...
						{row.original.isDrafted && (
							<span className="rounded-full bg-teal-100/60 dark:bg-teal-900/30 px-2 py-0.5 text-[10px] font-medium text-teal-700 dark:text-teal-300">
								{resolveTeamLabel(row.original.draftedTeamIndex)}
							</span>
						)}
						{row.original.isKeeper && (
							<span className="rounded-full bg-amber-100/60 dark:bg-amber-900/30 px-2 py-0.5 text-[10px] font-medium text-amber-700 dark:text-amber-300">
						...
						className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${
							row.original.player._type === "batter"
								...
								: row.original.player._type === "pitcher"
									? "bg-indigo-100/60 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300"
		...
		const withLeadingSeparator = (
			...
			const mergedClass = [existingClass, "border-l border-slate-200/40 dark:border-slate-600/20"]
				.filter(Boolean)
				.join(" ");
	...
	if (
		...
		return (
			<div className="flex h-96 flex-col items-center justify-center rounded-2xl border border-slate-200/50 dark:border-slate-600/30 backdrop-blur-xl bg-white/50 dark:bg-[#0f1729]/60">
				<p className="mb-2 text-lg font-light text-slate-400 dark:text-slate-500">No players loaded</p>
				<p className="text-sm font-light text-slate-300 dark:text-slate-600">Upload a CSV file to get started</p>
			</div>
		);
	}
	...
	return (
		<div className="space-y-3">
			<div className="overflow-x-auto rounded-2xl border border-slate-200/50 dark:border-slate-600/30 backdrop-blur-xl bg-white/70 dark:bg-[#0f1729]/80 shadow-xl shadow-teal-500/5">
				<table className="w-full text-sm text-[#1e293b] dark:text-[#cbd5e1]">
					<thead className="backdrop-blur-md bg-white/50 dark:bg-slate-800/40">
								{table.getHeaderGroups().map((headerGroup) => (
									<tr key={headerGroup.id}>
										...
										<th
											...
											className={`px-3 py-2.5 text-left text-[10px] font-medium uppercase tracking-widest text-slate-400 dark:text-slate-500 whitespace-nowrap ${
												header.column.getCanSort()
													? "cursor-pointer select-none hover:text-teal-600 dark:hover:text-teal-400"
													: ""
											} ${
													(
					...
					<tbody>
						{table.getRowModel().rows.map((row, index) => (
							<tr
								...
								className={`border-t border-slate-200/30 dark:border-slate-600/20 transition-colors ${
									...
									row.original.isDrafted
										...
										: row.original.isKeeper
											...
											: index % 2 === 0
												? "bg-transparent hover:bg-cyan-50/30 dark:hover:bg-cyan-900/10"
												: "bg-white/30 dark:bg-slate-800/20 hover:bg-cyan-50/30 dark:hover:bg-cyan-900/10"
								}`}
							>
			...
			<div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/50 dark:border-slate-600/30 backdrop-blur-xl bg-white/70 dark:bg-[#0f1729]/80 px-4 py-2.5 text-xs shadow-xl shadow-teal-500/5">
				<div className="flex items-center gap-2">
					<button
						onClick={() => table.previousPage()}
						disabled={!table.getCanPreviousPage()}
						className="rounded-xl border border-slate-200/60 dark:border-slate-600/40 backdrop-blur-md bg-white/50 dark:bg-slate-800/50 px-3 py-1 text-xs font-medium text-[#1e293b] dark:text-[#cbd5e1] hover:bg-white/80 dark:hover:bg-slate-700/50 disabled:cursor-not-allowed disabled:opacity-40 transition-all"
					>
						Prev
					</button>
					<button
				...
				<div className="flex items-center gap-3">
					...
					<label className="flex items-center gap-2">
						...
						<select
							...
							className="rounded-xl border border-slate-200/60 dark:border-slate-600/40 backdrop-blur-md bg-white/50 dark:bg-slate-800/50 px-2 py-1 text-xs text-[#1e293b] dark:text-[#cbd5e1] focus:border-teal-400 dark:focus:border-teal-400 focus:ring-2 focus:ring-teal-400/20 dark:focus:ring-teal-400/10"
						>
